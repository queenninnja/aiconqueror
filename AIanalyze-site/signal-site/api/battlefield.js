...
module.exports = async (req, res) => {
  ...
  try {
    const result = await askClaudeForJson(instruction, { maxTokens: 4096 });
    res.status(200).json(result);
  } catch (err) {
    console.error('battlefield failed:', err);
    const statusByCode = {
      missing_api_key: 500,
      rate_limited: 429,
      invalid_json: 502,
    ...
    res.status(statusByCode[code] || 500).json({ error: err.message || 'battlefield failed', code });
  }
};
